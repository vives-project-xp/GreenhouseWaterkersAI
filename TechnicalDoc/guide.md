...
The battery is housed in the bottom compartment of the case. The case allows access to the USB port for charging the battery and includes an opening for the on/off switch. The bottom part of the case has holes designed to accommodate threaded inserts. These inserts can be placed into the holes using a soldering iron, which melts them securely into position. These inserts are used to fasten the top part of the case to the bottom part.

The Raspberry Pi is secured to an intermediate plate, which also features holes for threaded inserts. These inserts allow the Raspberry Pi to be screwed onto the plate. The intermediate plate is then placed on top of the bottom section of the case. An opening is provided to connect a USB cable from the battery to the Raspberry Pi.

The Sense HAT is mounted on top of the Raspberry Pi using a 40-pin GPIO stacking header (female). Two spacer tubes are used to ensure the Sense HAT is level with both the GPIO pins and the top part of the case. The top part of the case is designed to attach to the bottom part using screws. It includes a cutout to make the LED matrix on the Sense HAT visible, along with a hole for pressing the button on the Sense HAT. Additionally, there’s an opening for the camera cable to connect to the Raspberry Pi’s USB port. Since the camera lacks a mount, it can be more easily aimed in the desired direction. The entire case is equipped with air vents to prevent components from overheating.

This prototype case is printed using PLA filament. However, for use in a greenhouse, we recommend printing it with PETG filament, as PLA is prone to deformation and discoloring due to humidity and UV exposure.